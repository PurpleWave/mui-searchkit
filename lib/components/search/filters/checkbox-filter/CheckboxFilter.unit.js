...
describe("CheckboxFilter tests", function () {
    ...
    it('clicks options', function () {
        ...
        expect(_this.wrapper).toMatchSnapshot("option is not active");
        TestHelpers_1.fastClick(option);
        expect(_this.wrapper).toMatchSnapshot("option is active");
    ...
    it("can disable", function () {
        expect(_this.wrapper).toMatchSnapshot("panel is not disabled");
        _this.searchkit.setResults({
            hits: { total: 0 },
            aggregations: {
                "test id1": {
        ...
        expect(_this.wrapper).toMatchSnapshot("panel is disabled");
        expect(_this.accessor.state.getValue()).toEqual(null);
        var option = _this.wrapper.find(".sk-item-list").children().at(0);
        TestHelpers_1.fastClick(option);
        expect(_this.accessor.state.getValue()).toEqual(true);
        expect(_this.wrapper).toMatchSnapshot("panel is not disabled again");
    });
});
