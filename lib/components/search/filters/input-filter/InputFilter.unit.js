...
describe("InputFilter tests", function () {
    ...
    it("toggles visibility", function () {
        ...
        _this.wrapper = _this.wrapper.update();
    ...
    it("search on change", function () {
        ...
        _this.wrapper.instance().throttledSearch.flush();
    ...
    it("search on change with clock", function () {
        ...
        _this.createWrapper(true);
        expect(_this.wrapper.instance().props.searchThrottleTime).toBe(200);
        _this.typeSearch("m");
        _this.wrapper.instance().throttledSearch.flush();
    ...
    it("should have a working remove icon", function () {
        ...
        _this.createWrapper(false);
        _this.setResults();
        expect(_this.wrapper.find(".sk-input-filter__remove").hasClass("is-hidden")).toBe(true);
        _this.typeSearch('ma');
        expect(_this.wrapper.find(".sk-input-filter__remove").hasClass("is-hidden")).toBe(false);
        expect(spy.callCount).toBe(0);
        _this.wrapper.find("form").simulate("submit");
        expect(spy.callCount).toBe(1);
    ...
    it("should accept Panel elements as containerComponent", function () {
        ...
        expect(_this.wrapper.find(".sk-panel__header").hasClass("is-collapsable")).toBe(true);
    });
    describe("url change + blurAction", function () {
        it("blurAction:restore", function () {
            ...
            _this.typeSearch("la");
            expect(_this.wrapper.instance().getValue()).toEqual("la");
            _this.accessor.fromQueryObject({
                test_id: "foo"
            });
            expect(_this.wrapper.instance().getValue()).toEqual("foo");
            _this.typeSearch("bar");
            expect(_this.wrapper.instance().getValue()).toEqual("bar");
            _this.wrapper.find(".sk-input-filter__text")
                .simulate("blur");
