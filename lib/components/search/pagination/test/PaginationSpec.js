...
describe("Pagination tests", function () {
    ...
    describe("rendering", function () {
        beforeEach(function () {
            _this.checkActionStates = function (page) {
        ...
        it('renders first page options', function () {
            _this.createWrapper();
            _this.checkActionStates(null);
            _this.createWrapper();
            expect(_this.wrapper).toMatchSnapshot("first page options");
        });
        it('renders second page options', function () {
            _this.createWrapper();
            _this.checkActionStates(2);
            _this.createWrapper();
            expect(_this.wrapper).toMatchSnapshot("second page options");
        });
        it('renders eighth page options', function () {
            _this.createWrapper();
            _this.checkActionStates(8);
            _this.createWrapper();
            expect(_this.wrapper).toMatchSnapshot("eighth page options");
        });
        it("handles showNumbers prop", function () {
            _this.createWrapper(false);
            _this.checkActionStates(4);
            _this.createWrapper(false);
            expect(_this.wrapper).toMatchSnapshot("show numbers props");
        });
        it("handles pageScope prop", function () {
            _this.createWrapper(true, 1);
            _this.checkActionStates(4);
            _this.createWrapper(true, 1);
            expect(_this.wrapper).toMatchSnapshot("page scope prop");
        });
        ...
        it("both disabled on only one total page", function () {
            ...
            _this.createWrapper();
            _this.checkActionStates(1);
            expect(_this.wrapper).toMatchSnapshot("both disabled on only one total page");
        });
    });
